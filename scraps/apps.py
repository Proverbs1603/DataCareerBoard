...
class ScrapsConfig(AppConfig):
    ...
    def ready(self):
        ...
        theTeamScraper.start_scraper_scheduler(interval_seconds=120)
        
        rocketpunchScraper = RocketpunchScraper()
        rocketpunchScraper.start_scraper_scheduler(interval_seconds=120)
        
        surfitScraper = SurfitScraper()
        surfitScraper.start_scraper_scheduler(interval_seconds=120)
        
        saraminScraper = SaraminScraper()
        saraminScraper.start_scraper_scheduler(interval_seconds=120)

        jobplanetScraper = JobplanetScraper()
        jobplanetScraper.start_scraper_scheduler(interval_seconds=120)

        wantedScraper = WantedScraper()
        wantedScraper.start_scraper_scheduler(interval_seconds=120)

        #스크래퍼 인스턴스 생성
        programmersScraper = ProgrammersScraper()
        # 60초마다 scrap 메서드 실행 #test10초
        programmersScraper.start_scraper_scheduler(interval_seconds=120)

        catchscraper = CatchScraper()
        catchscraper.start_scraper_scheduler(interval_seconds=120)

        peoplenJobScraper = PeoplenJobScraper()
        peoplenJobScraper.start_scraper_scheduler(interval_seconds=120)

        incruitScraper = IncruitScraper()
        incruitScraper.start_scraper_scheduler(interval_seconds=120)

# python manage.py runserver --noreload 실행 명령어
# python manage.py runserver 웹 페이지 테스트 시 실행 명령어